...
// [[Rcpp::export]]
arma::vec kee_cox_estequ(const arma::vec& beta, 
    ...
    if (delta[i] == 1.0 && kerval[i] > 0) {
      ...
      for (int k = 0; k < n; ++k) {
        if (X[i] <= X[k]) {
          ...
          if (dist_XX > 0) {
            double kerval_XX = std::max((1.0 - std::pow(dist_XX / h, 2.0)) * 0.75, 0.0) / h;
            if (kerval_XX > 0) {
...
// [[Rcpp::export]]
List kee_cox_var(const arma::vec& beta, 
                 const arma::mat& covariates, 
                 const arma::vec& X, 
                 const arma::vec& obs_times, 
                 const arma::vec& delta, 
        ...
        if (X[i] <= X[k]) {
          ...
          if (dist_XX > 0) {
            ...
            if (kerval_XX > 0) {
              ...
              S0_XX_sum += term;
              S1_XX_sum += term * trans(covariates.row(k));
              S2_XX_sum += term * (trans(covariates.row(k)) * covariates.row(k));
            }
          }
        }
      }
...
// [[Rcpp::export]]
List kee_additive_est(const arma::mat& covariates, 
                      const arma::vec& X, 
                      const arma::vec& obs_times, 
                      const arma::vec& delta, 
                      const arma::vec& kerval, 
  ...
  for (int i = 0; i < n; ++i) {
    if (kerval[i] > 0) {
      ...
      if (S0_XX_sum > 0) {
        arma::vec Zbar_XX = S1_XX_sum / S0_XX_sum;
        id_to_B[id[i]] += kerval[i] * (trans(covariates.row(i)) - Zbar_XX);
      }
    }
  }
  ...
  return List::create(Named("est") = beta,
                      Named("A_est") = A,
                      Named("B_est") = B,
                      Named("Sigma_est") = Sigma);
}
