...
estproc_Cox_Cao_mul <- function(data, n, nknots = NULL, norde = NULL, s = NULL, h, pl = 0) {
  ...
  Sigma_indi <- tibble(Sig = Sigma_indi_all, id = as.integer(id)) %>%
    group_by(id) %>%
    summarise(Sigma = t(colSums(Sig)))
...
estproc_ori_dh <- function(data, n, nknots, norder, s, h, initpara = NULL) {
  ...
  B <- function(beta, gamma) {
    ...
    bb1 <- tibble(a = bb1, id = as.numeric(id)) %>%
      group_by(id) %>%
      summarise(colSums(a))
    ...
    b_inner <- function(tt, beta, gamma) {
      ...
      for (jj in 1:nc) {
        ...
        temp1[is.na(temp1)] <- 0
        r1 <- tibble(a = t(temp1), id = as.numeric(id)) %>%
          group_by(id) %>%
          summarise(colMeans(a))
        mr1 <- NULL
...
hcv <- function(simdata, n, K, s) {
  ...
  res %>%
    bind_rows() %>%
    group_by(bd) %>%
    summarize(cvloss = mean(test_logll))
}
